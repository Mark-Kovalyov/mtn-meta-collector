...
public class MetaVisitor extends SimpleFileVisitor<Path> {
    ...
    public MetaVisitor(Model model, String inputFolder, boolean longJump) {
        super();
    ...
    private void processFileHeader(Path file, Resource idRes) {
        ...
        String localFilePath = file.toString().substring(inputFolder.length());
        model.add(
                idRes,
                model.createProperty("http://file.org#localFilePath"),
                localFilePath,
