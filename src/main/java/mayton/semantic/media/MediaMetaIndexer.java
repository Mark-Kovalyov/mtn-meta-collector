...
public class MediaMetaIndexer {

    static Logger logger = LoggerFactory.getLogger(MediaMetaIndexer.class);
    ...
    public static int collectMetaToDataSet(String inputFolder, Dataset dataset) throws IOException {
        ...
        MetaVisitor metaVisitor = new MetaVisitor(model, inputFolder, false);
    ...
    private static void modelReport(Model model) {
        ...
        model.listNameSpaces().toList().forEach(item -> logger.info(" Namespace : '{}'", item));
    ...
    public static void exportToFiles(Model model, String ttlOutputFile, String xmlOutputFile) {
        new File(new File(ttlOutputFile).getParent()).mkdirs();
        new File(new File(xmlOutputFile).getParent()).mkdirs();
    ...
    public static void main(String[] args) throws IOException {
        ...
        new File(tdbOutputFolder).mkdirs();
